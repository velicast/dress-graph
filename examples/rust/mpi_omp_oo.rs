// mpi_omp_oo.rs — Rook vs Shrikhande with Δ¹-DRESS (MPI + OpenMP, OO API)
//
...
// Add to Cargo.toml:
//   [dependencies]
//   dress-graph = { path = "../../rust", features = ["mpi", "omp"] }
//   mpi = "0.8"
//
// Run:
//   mpirun -np 4 cargo run --example mpi_omp_oo --features mpi,omp
...
fn main() {
    ...
    // Build persistent graph objects (MPI+OpenMP)
    let mut rook = mpi::DRESS::new(16, rook_s, rook_t, None, None, Variant::Undirected, false).unwrap();

    let mut shri = mpi::DRESS::new(16, shri_s, shri_t, None, None, Variant::Undirected, false).unwrap();
