// mpi_cuda_oo.rs — Rook vs Shrikhande with Δ¹-DRESS (MPI + CUDA, OO API)
//
...
// Add to Cargo.toml:
//   [dependencies]
//   dress-graph = { path = "../../rust", features = ["mpi", "cuda"] }
//   mpi = "0.8"
//
// Run:
//   mpirun -np 4 cargo run --example mpi_cuda_oo --features mpi,cuda
...
fn main() {
    ...
    // Build persistent graph objects (MPI+CUDA)
    let rook = mpi::cuda::DRESS::new(16, rook_s, rook_t, None, None, Variant::Undirected, false).unwrap();

    let shri = mpi::cuda::DRESS::new(16, shri_s, shri_t, None, None, Variant::Undirected, false).unwrap();
