// omp_oo.rs — Prism vs K₃,₃ with DRESS (OpenMP, OO API)
//
// Demonstrates the persistent DRESS object with GPU-accelerated fitting.
//
// Add to Cargo.toml:
//   [dependencies]
//   dress-graph = { path = "../../rust", features = ["omp"] }
//
// Run:
//   cargo run --example omp_oo --features omp
use dress_graph::{omp, Variant};
...
fn main() {
    ...
    let mut prism = omp::DRESS::new(6, prism_s, prism_t, None, None, Variant::Undirected, false).unwrap();

    let mut k33 = omp::DRESS::new(6, k33_s, k33_t, None, None, Variant::Undirected, false).unwrap();
