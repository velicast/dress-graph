// mpi_omp.rs — Rook vs Shrikhande with Δ¹-DRESS (MPI + OpenMP)
...
//
// Run:
//   mpirun -np 4 cargo run --example mpi_omp --features "mpi omp"
...
fn main() {
    ...
    let dr = mpi::omp::delta_fit(16, rook_s, rook_t, None, None, 1, 100, 1e-6, 0, 0,
                                   Variant::Undirected, false, true, true, &world).unwrap();
    let ds = mpi::omp::delta_fit(16, shri_s, shri_t, None, None, 1, 100, 1e-6, 0, 0,
                                   Variant::Undirected, false, true, true, &world).unwrap();
