# mpi_omp.R — Rook vs Shrikhande with Δ¹-DRESS (MPI + OpenMP)
# Keeps multisets and compares them to guarantee distinguishability.
#
# Run:
#   mpirun -np 4 Rscript mpi_omp.R
library(dress.graph)
library(pbdMPI)
...
dr <- mpi$omp$delta_fit(16L, rook_s, rook_t, k = 1L, keep_multisets = TRUE)
ds <- mpi$omp$delta_fit(16L, shri_s, shri_t, k = 1L, keep_multisets = TRUE)
