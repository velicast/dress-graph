# mpi_cuda.R — Rook vs Shrikhande with Δ¹-DRESS (MPI + CUDA)
# Keeps multisets and compares them to guarantee distinguishability.
#
# Run:
#   mpirun -np 4 Rscript mpi_cuda.R
library(dress.graph)
library(pbdMPI)
...
dr <- mpi$cuda$delta_dress_fit(16L, rook_s, rook_t, k = 1L, keep_multisets = TRUE)
ds <- mpi$cuda$delta_dress_fit(16L, shri_s, shri_t, k = 1L, keep_multisets = TRUE)
