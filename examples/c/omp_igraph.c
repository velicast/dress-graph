/* omp_igraph.c — Prism vs K₃,₃ with DRESS (CUDA, igraph backend)
 *
 * Same comparison as cuda.c but using igraph_t graphs and the
 * CUDA igraph wrapper.  Including dress/omp/igraph/dress.h redirects
 * dress_fit() to the CUDA implementation.
 *
 * Build:
 *   gcc -O2 -o cuda_igraph omp_igraph.c -ldress \
