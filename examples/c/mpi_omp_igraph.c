/* mpi_omp_igraph.c — Rook vs Shrikhande with Δ¹-DRESS (MPI + OpenMP, igraph)
 *
 * Same comparison as mpi_igraph.c but using the CUDA + MPI backend.
 * Including dress/mpi/omp/igraph/dress.h redirects dress_delta_fit()
 * to the GPU + MPI implementation — no source changes required.
 *
 * Build & run:
 *   mpicc -O2 -fopenmp -o mpi_cuda_igraph mpi_omp_igraph.c \
 *       -ldress \
...
#include <dress/mpi/omp/igraph/dress.h>
