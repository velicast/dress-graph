 ...
 */
import { deltaFit } from 'dress-graph';
...
const dr = await deltaFit({ numVertices: 16, sources: rook_s, targets: rook_t, k: 1, keepMultisets: true });
const ds = await deltaFit({ numVertices: 16, sources: shri_s, targets: shri_t, k: 1, keepMultisets: true });

console.log(`Rook:       ${dr.histogram.length} histogram entries, ${dr.numSubgraphs} subgraphs`);
console.log(`Shrikhande: ${ds.histogram.length} histogram entries, ${ds.numSubgraphs} subgraphs`);

// Compare histograms
const histSame = dr.histogram.length === ds.histogram.length &&
    dr.histogram.every((entry, i) => entry.value === ds.histogram[i].value && entry.count === ds.histogram[i].count);
