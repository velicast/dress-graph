// cuda.cpp — Prism vs K₃,₃ with DRESS (CUDA, C++ wrapper)
//
// Build:
//   g++ -O2 -std=c++17 -o cuda cuda.cpp -ldress -ldress_cuda -lm
#include <iostream>
#include <algorithm>
#include <vector>
...
int main() {
    ...
    cuda::DRESS prism(6, pU, pV);
    cuda::DRESS k33(6, kU, kV);

    prism.fit(100, 1e-6);
    k33.fit(100, 1e-6);

    auto fp = [](cuda::DRESS& g) {
