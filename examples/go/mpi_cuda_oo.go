// mpi_cuda_oo.go — Rook vs Shrikhande with Δ¹-DRESS (MPI + CUDA, OO API)
//
// Demonstrates the persistent DRESS object with GPU fitting and
// MPI-distributed DeltaFit.
//
// Run:
//
//	mpirun -np 4 go run mpi_cuda_oo.go
package main
...
import (
	...
	dress "github.com/velicast/dress-graph/go/mpi/cuda"
)
...
func main() {
	...
	rook, _ := dress.NewDRESS(16, rookS, rookT, nil, nil, dress.Undirected, false)
	defer rook.Close()
	shri, _ := dress.NewDRESS(16, shriS, shriT, nil, nil, dress.Undirected, false)
	defer shri.Close()


	// MPI+CUDA distributed Δ¹-DRESS
	dr, _ := rook.DeltaFit(1, 100, 1e-6, 0, 0, true, true)
	ds, _ := shri.DeltaFit(1, 100, 1e-6, 0, 0, true, true)
