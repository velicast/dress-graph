...
Individual subgraph fingerprints are not recoverable from the pooled histogram. The memory footprint is constant regardless of $k$ - a single fixed-size integer array - whereas storing the raw multiset of $\binom{n}{k} \cdot |E|$ floating-point values would be prohibitive. This fixed-size representation, combined with the deletion operator, empirically matches the discriminative power of $(k{+}2)$-WL methods that require $\mathcal{O}(n^{k+2})$ storage.
...
- **$\Delta^1$** extends the range to CFI($K_4$) (3-WL), consistent with Δ-DRESS distinguishing SRG pairs that confound 2-WL.
