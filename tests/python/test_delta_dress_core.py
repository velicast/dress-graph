"""
Tests for the pure-Python Δ^k-DRESS implementation (dress.core.delta_fit).
...
from dress.core import delta_fit, DeltaDRESSResult, Variant
...
def _values(r):
    return [value for value, _ in r.histogram]
...
class TestHistSize:
    def test_result_type(self):
        ...
        # Map size is number of unique values
    ...
    def test_weighted_hist_size(self):
        ...
        r = delta_fit(3, K3_SRC, K3_TGT,
                            ...
                            k=0, epsilon=1e-3)
        assert len(r.histogram) > 1, f"expected > 1 unique value"
        assert _total(r) == 3
...
class TestDelta0:
    ...
    def test_k3_single_bin(self):
        """K3 is vertex-transitive → all edges have same value."""
        r = delta_fit(3, K3_SRC, K3_TGT, k=0, epsilon=EPS)
        assert len(r.histogram) == 1

    def test_k3_top_bin(self):
        """K3 edges have dress ≈ 2.0."""
        r = delta_fit(3, K3_SRC, K3_TGT, k=0, epsilon=EPS)
        # All edges approx 2.0
        for value in _values(r):
            assert abs(value - 2.0) < 1e-2
...
class TestEdgeCases:
    def test_k_eq_N(self):
        r = delta_fit(3, K3_SRC, K3_TGT, k=3, epsilon=EPS)
        assert _total(r) == 0

    def test_k_gt_N(self):
        r = delta_fit(3, K3_SRC, K3_TGT, k=10, epsilon=EPS)
        assert _total(r) == 0

    def test_single_edge_delta0(self):
        r = delta_fit(2, [0], [1], k=0, epsilon=EPS)
        assert _total(r) == 1

    def test_single_edge_delta1(self):
        """Removing either endpoint of a single-edge graph → 0 edges."""
        r = delta_fit(2, [0], [1], k=1, epsilon=EPS)
        assert _total(r) == 0
...
class TestPrecompute:
    def test_identical_results(self):
        ...
        r2 = delta_fit(4, K4_SRC, K4_TGT, k=1, epsilon=EPS,
                             precompute=True)
        assert len(r1.histogram) == len(r2.histogram)
...
class TestInitDispatch:
    def test_top_level_import(self):
        from dress import delta_fit as top_delta
