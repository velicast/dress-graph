...
Strongly Regular Graphs (SRGs) are the canonical hard instances for polynomial-time isomorphism methods: all edges share identical local structure (same degree, same common-neighbor counts), so Original-DRESS ($\Delta^0$) assigns the same value to every edge and produces a uniform fingerprint. This is expected: SRGs are regular, and the triangle-based DRESS iteration sees no local variation.
