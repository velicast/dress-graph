...
#' distribution by exhaustively individualizing all k-vertex subsets
#' (multiplying incident edge weights by \code{nabla_weight}) and
#' measuring the resulting edge similarity values.
#'
#' @param n_vertices Integer. Number of vertices (vertex ids in 0..N-1).
#' @param sources Integer vector of length E — edge source endpoints (0-based).
#' @param targets Integer vector of length E — edge target endpoints (0-based).
#' @param weights Optional numeric vector of length E (default NULL = unweighted).
#' @param k Integer. Number of vertices to individualize (0 = original graph).
#' @param nabla_weight Numeric. Multiplicative factor for incident edges (default 2.0).
#' @param variant Graph variant (default \code{DRESS_UNDIRECTED}).
#' @param max_iterations Maximum fitting iterations (default 100).
#' @param epsilon Convergence threshold (default 1e-6).
#' @param precompute Logical. Pre-compute intercept index (default FALSE).
...
#' @return A list with components:
#' \describe{
#'   \item{\code{histogram}}{Numeric vector — bin counts of edge values.}
#'   \item{\code{hist_size}}{Integer — number of bins (floor(2/epsilon) + 1).}
#' }
#'
#' @examples
#' # Triangle K3, nabla-1
#' res <- nabla_dress_fit(3L, c(0L,1L,2L), c(1L,2L,0L), k = 1L)
#' res$hist_size
#'
#' @useDynLib dress.graph, .registration = TRUE
#' @export
nabla_dress_fit <- function(n_vertices,
                            sources,
                            targets,
                            weights          = NULL,
                            k                = 0L,
  ...
  if (!is.null(weights)) weights <- as.double(weights)
  k              <- as.integer(k)
  nabla_weight   <- as.double(nabla_weight)
  variant        <- as.integer(variant)
  max_iterations <- as.integer(max_iterations)
  epsilon        <- as.double(epsilon)
  precompute     <- as.integer(precompute)
