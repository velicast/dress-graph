...
const _FN_NABLA_MPI_CUDA_FCOMM = Ref{Ptr{Cvoid}}(C_NULL)
...
function _ensure_lib()
    ...
    sym = dlsym_e(_LIB_CUDA[], :dress_delta_fit_mpi_cuda_fcomm)
    sym == C_NULL && error(
        "dress_delta_fit_mpi_cuda_fcomm not found in libdress_cuda. " *
        "Ensure libdress_cuda.so was built with MPI support."
    )
    _FN_MPI_CUDA_FCOMM[] = sym

    sym_nabla = dlsym_e(_LIB_CUDA[], :dress_nabla_fit_mpi_cuda_fcomm)
    sym_nabla == C_NULL && error(
        "dress_nabla_fit_mpi_cuda_fcomm not found in libdress_cuda. " *
        "Ensure libdress_cuda.so was built with MPI support."
    )
    _FN_NABLA_MPI_CUDA_FCOMM[] = sym_nabla
end
...
MPI-distributed ∇^k-DRESS (CUDA backend).  Each rank runs GPU-accelerated
DRESS on its stride of tuples.
...
function nabla_fit(N::Integer,
    ...
    h_ptr = ccall(_FN_NABLA_MPI_CUDA_FCOMM[], Ptr{HistogramEntry},
                  (Ptr{Cvoid}, Cint, Cint, Cdouble, Cint, Cuint, Ptr{Cint}, Cint,
                   Ptr{Ptr{Cdouble}}, Ptr{Int64}, Cint),
                  g, Cint(k), Cint(max_iterations), Cdouble(epsilon),
                  Cint(n_samples), Cuint(seed),
...
MPI+CUDA distributed ∇^k-DRESS on a persistent graph.
"""
