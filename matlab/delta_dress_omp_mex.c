/*
 * delta_dress_omp_mex.c — MATLAB MEX gateway for delta-k-DRESS.
 *
 * Compile from the matlab/ directory:
 *
 *   mex -O -I../libdress/include CFLAGS="$CFLAGS -fopenmp" LDFLAGS="$LDFLAGS -fopenmp" ...
 *       delta_dress_omp_mex.c ../libdress/src/dress.c ../libdress/src/delta_dress.c -lm
 *
 * Usage in MATLAB:
 *
 *   result = delta_dress_omp_mex(n_vertices, sources, targets, ...
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 8 || nrhs > 12)
        mexErrMsgIdAndTxt("delta_dress:nrhs",
            ...
            "  delta_dress_omp_mex(n_vertices, sources, targets, weights, k, "
    ...
    hist = dress_delta_fit_omp_strided(g, k, max_iterations, epsilon,
                    n_samples, seed,
