/*
 * delta_dress_mpi_cuda_obj_mex.c — MEX gateway: MPI+CUDA delta-fit on a persistent DRESS graph.
 *
 * Usage:
 *   result = delta_dress_mpi_cuda_obj_mex(ptr, k, max_iterations, epsilon, keep_multisets)
 */
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 5)
        mexErrMsgIdAndTxt("dress_mpi_cuda:nrhs",
            "Five inputs required: delta_dress_mpi_cuda_obj_mex(ptr, k, max_iterations, epsilon, keep_multisets)");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("dress_mpi_cuda:nlhs", "At most one output.");

    g = (p_dress_graph_t)(uintptr_t)(*((uint64_t *)mxGetData(prhs[0])));
    if (!g)
        mexErrMsgIdAndTxt("dress_mpi_cuda:nullPtr", "Graph pointer is NULL (already closed?).");
    ...
    if (k < 0)
        mexErrMsgIdAndTxt("dress_mpi_cuda:invalidK", "k must be >= 0.");

    dress_mpi_init();
    hist = dress_delta_fit_mpi_cuda_world(g, k, max_iterations, epsilon,
                                          n_samples, seed,
