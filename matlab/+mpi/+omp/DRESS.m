...
% MPI.OMP.DRESS  Persistent MPI+OMP DRESS graph object.
%
%   g = mpi.omp.DRESS(n_vertices, sources, targets)
%   g = mpi.omp.DRESS(n_vertices, sources, targets, 'Weights', w, ...)
%
%   Methods:
%     res = g.delta_fit('K', 2)          — MPI+OMP Δ^k-DRESS
%     res = g.nabla_fit('K', 2)          — MPI+OMP ∇^k-DRESS
...
%
%   Example:
%     g = mpi.omp.DRESS(4, int32([0;1;2;2]), int32([1;2;0;3]));
%     dr = g.delta_fit('K', 1);
%     g.close();
    ...
    methods
        ...
        % DELTA_FIT  MPI+OMP distributed Δ^k-DRESS on this persistent graph.
            p = inputParser;
            ...
            result = delta_dress_mpi_omp_obj_mex(obj.ptr, ...
                                                  int32(p.Results.K), ...
        ...
        % NABLA_FIT  MPI+OMP distributed ∇^k-DRESS on this persistent graph.
            p = inputParser;
            addParameter(p, 'K',              0,     @(x) isscalar(x) && isnumeric(x) && x >= 0);
            addParameter(p, 'MaxIterations',  100,   @(x) isscalar(x) && isnumeric(x) && x >= 1);
            addParameter(p, 'Epsilon',        1e-6,  @(x) isscalar(x) && isnumeric(x) && x > 0);
            ...
            result = nabla_dress_mpi_omp_obj_mex(obj.ptr, ...
                                                  int32(p.Results.K), ...
                                                  int32(p.Results.MaxIterations), ...
                                                  double(p.Results.Epsilon), ...
                                                  int32(logical(p.Results.KeepMultisets)));
