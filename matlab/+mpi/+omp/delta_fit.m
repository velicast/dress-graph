...
% MPI.OMP.DELTA_DRESS_FIT  MPI+OpenMP distributed Delta-k-DRESS exact histogram.
%
%   result = mpi.omp.delta_fit(n_vertices, sources, targets)
%   result = mpi.omp.delta_fit(n_vertices, sources, targets, Name, Value, ...)
%
%   Same API as delta_fit(), but subgraph enumeration is distributed
%   across MPI ranks and each subgraph fitting is multi-threaded via OpenMP.
...
%     r = mpi.omp.delta_fit(4, sources, targets, 'K', 1);
%
%   Required inputs:
...
%
%   See also: delta_fit, mpi.omp.DRESS, delta_dress_mpi_omp_mex
    ...
    result = delta_dress_mpi_omp_mex(n_vertices, sources, targets, weights, k, ...
