...
% MPI.OMP.NABLA_DRESS_FIT  MPI+OpenMP distributed Nabla-k-DRESS exact histogram.
%
%   result = mpi.omp.nabla_fit(n_vertices, sources, targets)
%   result = mpi.omp.nabla_fit(n_vertices, sources, targets, Name, Value, ...)
%
%   Same API as nabla_fit(), but tuple enumeration is distributed
%   across MPI ranks and each tuple fitting is multi-threaded via OpenMP.
%   Switch from CPU to MPI+OMP by adding the mpi.omp. prefix:
%
%     % CPU
%     r = nabla_fit(4, sources, targets, 'K', 1);
%
%     % MPI+OMP (same call, different namespace)
%     r = mpi.omp.nabla_fit(4, sources, targets, 'K', 1);
%
%   Required inputs:
...
%
%   See also: nabla_fit, mpi.omp.DRESS, nabla_dress_mpi_omp_mex
    ...
    assert(isscalar(n_vertices) && n_vertices >= 1, ...
           'mpi_omp:invalidInput', 'n_vertices must be a positive scalar.');
    assert(numel(sources) == numel(targets), ...
           'mpi_omp:invalidInput', 'sources and targets must have the same length.');
    assert(variant >= 0 && variant <= 3, ...
           'mpi_omp:invalidInput', 'Variant must be 0, 1, 2, or 3.');
    if ~isempty(weights)
        assert(numel(weights) == numel(sources), ...
               'mpi_omp:invalidInput', 'weights must have the same length as sources.');
    end
    ...
    result = nabla_dress_mpi_omp_mex(n_vertices, sources, targets, weights, k, ...
