...
% MPI.CUDA.NABLA_DRESS_FIT  MPI+CUDA distributed Nabla-k-DRESS exact histogram.
%
%   result = mpi.cuda.nabla_fit(n_vertices, sources, targets)
%   result = mpi.cuda.nabla_fit(n_vertices, sources, targets, Name, Value, ...)
%
%   Same API as nabla_fit(), but tuple enumeration is distributed
%   across MPI ranks and each tuple fitting runs on the GPU via CUDA.
...
%     r = mpi.cuda.nabla_fit(4, sources, targets, 'K', 1);
%
%   Required inputs:
...
%
%   See also: nabla_fit, mpi.cuda.DRESS, nabla_dress_mpi_cuda_mex
    ...
    result = nabla_dress_mpi_cuda_mex(n_vertices, sources, targets, weights, k, ...
