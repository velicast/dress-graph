...
% MPI.NABLA_DRESS_FIT  MPI-distributed Nabla-k-DRESS exact histogram.
%
%   result = mpi.nabla_fit(n_vertices, sources, targets)
%   result = mpi.nabla_fit(n_vertices, sources, targets, Name, Value, ...)
%
%   Same API as nabla_fit(), but tuple enumeration is distributed
%   across MPI ranks.  Switch from CPU to MPI by adding the mpi. prefix:
%
%     % CPU
%     r = nabla_fit(4, sources, targets, 'K', 1);
%
%     % MPI (same call, different namespace)
%     r = mpi.nabla_fit(4, sources, targets, 'K', 1);
%
%   Required inputs:
...
%
%   See also: nabla_fit, mpi.DRESS, nabla_dress_mpi_mex
    ...
    assert(isscalar(n_vertices) && n_vertices >= 1, ...
           'mpi:invalidInput', 'n_vertices must be a positive scalar.');
    assert(numel(sources) == numel(targets), ...
           'mpi:invalidInput', 'sources and targets must have the same length.');
    assert(variant >= 0 && variant <= 3, ...
           'mpi:invalidInput', 'Variant must be 0, 1, 2, or 3.');
    if ~isempty(weights)
        assert(numel(weights) == numel(sources), ...
               'mpi:invalidInput', 'weights must have the same length as sources.');
    end
    ...
    result = nabla_dress_mpi_mex(n_vertices, sources, targets, weights, k, ...
