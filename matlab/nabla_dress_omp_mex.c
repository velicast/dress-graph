/*
 * nabla_dress_omp_mex.c — MATLAB MEX gateway for nabla-k-DRESS.
 *
 * Compile from the matlab/ directory:
 *
 *   mex -O -I../libdress/include CFLAGS="$CFLAGS -fopenmp" LDFLAGS="$LDFLAGS -fopenmp" ...
 *       nabla_dress_omp_mex.c ../libdress/src/dress.c ../libdress/src/delta_dress.c -lm
 *
 * Usage in MATLAB:
 *
 *   result = nabla_dress_omp_mex(n_vertices, sources, targets, ...
 ...
 *     .multisets      — double [P(N,k) x E]     per-tuple edge values (NaN = removed)
 ...
 *   k                — scalar int: vertices to individualize (0 = original graph)
 ...
 *            nonzero: .multisets (double [P(N,k) x E]) and
 *            .num_tuples (int32).
 */
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 8 || nrhs > 12)
        mexErrMsgIdAndTxt("nabla_dress:nrhs",
            ...
            "  nabla_dress_omp_mex(n_vertices, sources, targets, weights, k, "
    ...
    hist = dress_nabla_fit_omp(g, k, max_iterations, epsilon,
                    n_samples, seed,
