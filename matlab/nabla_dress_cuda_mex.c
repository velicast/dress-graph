/*
 * nabla_dress_cuda_mex.c — MATLAB MEX gateway for GPU-accelerated Δ^k-DRESS.
 *
 * Same interface as nabla_dress_mex.c but calls dress_nabla_fit_cuda().
 */
...
static double get_scalar_double(const mxArray *arg, const char *name)
{
    if (!mxIsDouble(arg) || mxIsComplex(arg) || mxGetNumberOfElements(arg) != 1)
        mexErrMsgIdAndTxt("nabla_dress_cuda:invalidInput",
...
static int *to_int_array(const mxArray *arg, int expected_len, const char *name)
{
    ...
    if (!out)
        mexErrMsgIdAndTxt("nabla_dress_cuda:malloc", "malloc failed for %s.", name);
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs < 8 || nrhs > 10)
        mexErrMsgIdAndTxt("nabla_dress_cuda:nrhs",
            "8 to 10 inputs required:\n"
            "  nabla_dress_cuda_mex(n_vertices, sources, targets, weights, k, "
            "variant, max_iterations, epsilon, precompute, keep_multisets)");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("nabla_dress_cuda:nlhs", "At most one output.");
    ...
    if (!mxIsEmpty(prhs[3])) {
        ...
        if (!W) { free(U); free(V); mexErrMsgIdAndTxt("nabla_dress_cuda:oom", "malloc failed"); }
    ...
    if (variant_val < 0 || variant_val > 3) {
        ...
        mexErrMsgIdAndTxt("nabla_dress_cuda:invalidVariant",
                          "variant must be 0..3.");
    }
    if (k < 0) {
        free(U); free(V); free(W);
        mexErrMsgIdAndTxt("nabla_dress_cuda:invalidK", "k must be >= 0.");
    }

    g = dress_init_graph(N, E, U, V, W,
                         NULL, (dress_variant_t)variant_val, precompute);
    if (!g)
        mexErrMsgIdAndTxt("nabla_dress_cuda:initFailed",
                          "dress_init_graph returned NULL.");

    hist = dress_nabla_fit_cuda(g, k, max_iterations, epsilon,
                                n_samples, seed,
    ...
    const char *fields_full[]   = { "histogram", "multisets", "num_tuples" };
    ...
    if (keep_ms && ms_ptr) {
        mxArray *m_ms = mxCreateDoubleMatrix((mwSize)num_tup, (mwSize)E, mxREAL);
        {
