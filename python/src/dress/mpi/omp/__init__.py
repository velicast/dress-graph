"""``dress.mpi.omp`` — MPI-distributed DRESS (OpenMP backend).

Switch to the MPI+OMP backend by changing the import::

    from dress.mpi.omp import DRESS

    g = DRESS(N, sources, targets)
    g.fit()           # OMP edge-parallel (single graph)
    g.delta_fit(k=3)  # MPI-distributed, each rank uses OMP subgraph-parallel
    g.get(0, 1)       # CPU

Requires ``libdress.so`` built with ``-DDRESS_MPI=ON`` and OpenMP,
plus ``mpi4py``.
...
from dress.core import DeltaDRESSResult, NablaDRESSResult, FitResult
from dress._ctypes_helpers import (
    _DressGraph, _DressHistPair, _p_dress_graph_t, _malloc_array,
)

# ---------------------------------------------------------------------------
#  Lazy-load libdress.so (with MPI + OpenMP support)
# ---------------------------------------------------------------------------
...
def _get_lib():
    """Load libdress.so (with MPI + OMP) on first use."""
    global _lib
    if _lib is not None:
        return _lib
    ...
    try:
        lib.dress_delta_fit_mpi_omp_fcomm
    except AttributeError:
        raise RuntimeError(
            "libdress.so found but MPI+OMP API not available. "
            "Rebuild with -DDRESS_MPI=ON and an OpenMP-capable compiler."
        )
    ...
    lib.dress_delta_fit_mpi_omp_fcomm.restype = ctypes.POINTER(_DressHistPair)
    lib.dress_delta_fit_mpi_omp_fcomm.argtypes = [
    ...
    lib.dress_nabla_fit_mpi_omp_fcomm.restype = ctypes.POINTER(_DressHistPair)
    lib.dress_nabla_fit_mpi_omp_fcomm.argtypes = [
        _p_dress_graph_t, ctypes.c_int, ctypes.c_int, ctypes.c_double,
        ctypes.c_int, ctypes.c_uint,
        ctypes.POINTER(ctypes.c_int), ctypes.c_int,
        ctypes.POINTER(ctypes.POINTER(ctypes.c_double)),
...
def delta_fit(
    ...
    hist_ptr = lib.dress_delta_fit_mpi_omp_fcomm(
...
def nabla_fit(
    ...
    MPI distributes tuples across ranks.  Within each rank, OpenMP
    threads further parallelise the tuple slice.
    ...
    hist_ptr = lib.dress_nabla_fit_mpi_omp_fcomm(
        g, k, max_iterations, ctypes.c_double(epsilon),
        n_samples, ctypes.c_uint(seed),
        ctypes.byref(hist_size) if compute_histogram else None,
        1 if keep_multisets else 0,
...
# ---------------------------------------------------------------------------
#  DRESS class — same API, delta_fit runs MPI+OMP, fit runs OMP
# ---------------------------------------------------------------------------

from dress.omp import DRESS as _OmpDRESS  # noqa: E402


class DRESS(_OmpDRESS):
    """MPI+OMP DRESS — ``fit()`` uses OMP, ``delta_fit()`` uses MPI+OMP.
    ...
    Usage::
        ...
        g.fit()               # OMP edge-parallel
        dr = g.delta_fit(k=3) # MPI + OMP subgraph-parallel
    """
    ...
    def __repr__(self):
        return (
            ...
            f"variant={self._var.name}, backend=mpi+omp)"
        )
