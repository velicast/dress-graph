// Package dress provides OpenMP-parallel Go bindings for the DRESS C library.
//
...
//
//	// OpenMP — same call, just change the import
//	import dress "github.com/velicast/dress-graph/go/omp"
//	result, _ := dress.Fit(4, U, V, nil, dress.Undirected, 100, 1e-6, true)
//
// # Build requirements
//
// Requires CGo, a C compiler, and the OpenMP support (-fopenmp).
// The OpenMP kernel object must be built first (make -C libdress/src/cuda).
package dress

/*
#cgo CFLAGS:  -O3 -DDRESS_OpenMP -Ivendor/include -Ivendor/src -I../../libdress/include -I../../libdress/src
...
// Fit runs the OpenMP-parallel DRESS iterative fitting algorithm.
//
// Same signature and semantics as dress.Fit() but the fitting loop runs
// on the OpenMP.
func Fit(n int, sources, targets []int32, weights []float64, vertexWeights []float64,
	...
	if vertexWeights != nil && len(vertexWeights) != n {
		return nil, fmt.Errorf("cuda: vertex weights length (%d) != node count (%d)", len(vertexWeights), n)
	}
	...
	var delta C.double
	C.dress_fit_omp(g, C.int(maxIterations), C.double(epsilon), &iterations, &delta)
	...
	if cnwPtr != nil {
		result.VertexWeights = make([]float64, n)
		nwSlice := unsafe.Slice(cnwPtr, n)
		for i := 0; i < n; i++ {
			result.VertexWeights[i] = float64(nwSlice[i])
		}
	}
	copy(result.Sources, sources)
	copy(result.Targets, targets)
...
// DeltaFit runs OpenMP-parallel Δ^k-DRESS.
//
// Same signature and semantics as dress.DeltaFit() but each subgraph
// fitting runs on the OpenMP.
func DeltaFit(n int, sources, targets []int32, weights []float64, vertexWeights []float64,
	...
	hPtr := C.dress_delta_fit_omp_strided(g, C.int(k), C.int(maxIterations),
...
func NablaFit(n int, sources, targets []int32, weights []float64, vertexWeights []float64,
	...
	hPtr := C.dress_nabla_fit_omp(g, C.int(k), C.int(maxIterations),
		C.double(epsilon), C.int(nSamples), C.uint(seed),
		func() *C.int {
			if computeHistogram {
				return &histSize
...
// .Get() queries without rebuilding.  Fitting uses OpenMP on the GPU.
type DRESS struct {
...
// Fit runs the OpenMP-accelerated DRESS iterative fitting algorithm.
func (dg *DRESS) Fit(maxIterations int, epsilon float64) (iterations int, delta float64, err error) {
	...
	var iters C.int
	var d C.double
	C.dress_fit_omp((C.p_dress_graph_t)(dg.g), C.int(maxIterations), C.double(epsilon), &iters, &d)
	return int(iters), float64(d), nil
}

// DeltaFit runs OpenMP-parallel Δ^k-DRESS on this persistent graph.
func (dg *DRESS) DeltaFit(k int, maxIterations int, epsilon float64,
	...
	hPtr := C.dress_delta_fit_omp_strided((C.p_dress_graph_t)(dg.g), C.int(k), C.int(maxIterations),
		C.double(epsilon), C.int(nSamples), C.uint(seed),
		func() *C.int {
			if computeHistogram {
				return &histSize
...
// NablaFit runs OpenMP-parallel ∇^k-DRESS on this persistent graph.
func (dg *DRESS) NablaFit(k int, maxIterations int, epsilon float64,
	...
	hPtr := C.dress_nabla_fit_omp((C.p_dress_graph_t)(dg.g), C.int(k), C.int(maxIterations),
		C.double(epsilon), C.int(nSamples), C.uint(seed),
		func() *C.int {
			if computeHistogram {
				return &histSize
...
func (dg *DRESS) Result() (*Result, error) {
	...
	if cnwPtr != nil {
		result.VertexWeights = make([]float64, dg.n)
